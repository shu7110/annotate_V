...
### 3. YOLO-Worldモデルのダウンロード

YOLO-Worldモデルは[Ultralyticsの公式ドキュメント](https://docs.ultralytics.com/ja/models/yolo-world/#available-models-supported-tasks-and-operating-modes)から入手できます。

現在使用しているモデル:
- **yolov8x-worldv2.pt** (140MB) - 最高精度モデル

#### 自動ダウンロード（推奨）
サーバー起動時に自動的にダウンロードされます。

#### 手動ダウンロード
```bash
# 直接ダウンロード（オプション）
curl -L -o yolov8x-worldv2.pt https://github.com/ultralytics/assets/releases/download/v8.3.0/yolov8x-worldv2.pt
```

**利用可能なモデルサイズ:**
| モデル | ファイル名 | サイズ | 用途 |
|--------|------------|--------|------|
| YOLOv8s-worldv2 | yolov8s-worldv2.pt | ~40MB | 軽量・高速 |
| YOLOv8m-worldv2 | yolov8m-worldv2.pt | ~80MB | バランス |
| YOLOv8l-worldv2 | yolov8l-worldv2.pt | ~110MB | 高精度 |
| YOLOv8x-worldv2 | yolov8x-worldv2.pt | ~140MB | 最高精度 |
